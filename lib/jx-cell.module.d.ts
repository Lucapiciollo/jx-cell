...
import * as i1 from "./components/jx-spreadsheet/jx-spreadsheet.component";
...
/**
 * Angular NgModule that provides the `jx-cell` spreadsheet library.
 *
 * Import `JxCellModule` once in your application's `AppModule` (or any feature
 * module where the grid is used) to make the `<jx-spreadsheet>` and `<jx-grid>`
 ...
 * own `providers` array — each `<jx-spreadsheet>` element gets its own isolated
